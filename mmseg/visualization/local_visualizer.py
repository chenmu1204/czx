...
class SegLocalVisualizer(Visualizer):
    ...
    def add_datasample(
        ...
        if gt_img_data is not None and pred_img_data is not None:
            drawn_img = np.concatenate((image, gt_img_data), axis=1)
            drawn_img = np.concatenate((drawn_img, pred_img_data), axis=1)   # 横向拼接：图像、gt、pre三张图片
        ...
        if out_file is not None:
            mmcv.imwrite(mmcv.rgb2bgr(drawn_img), out_file)           # 保存可视化的gt和pre
        else:
