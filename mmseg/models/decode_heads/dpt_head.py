...
class DPTHead(BaseDecodeHead):
    ...
    def __init__(self,
        ...
        '''思路1：自定义卷积层'''
        channels1 = [256, 256, 256, 256]
        channels2 = [256, 256, 256, 256]
    ...
    def forward(self, inputs):
        ...
        x = [self.convs[i](feature) for i, feature in enumerate(x)]   # 4个层次的特征图
        ...
        for idx in range(len(x) - 1):
            idx = 3 - idx        # [3,2,1]
            x1 = x[idx]     # (2,256,16,16)
            x2 = x[idx - 1] # (2,256,32,32)
            # conv = self.convs_enhance[idx]  # 256→160，（16,16,160）
            x1 = resize(input=x1, size=x2.shape[2:], mode='bilinear', align_corners=False)  # 先改变通道数，再resize
            ...
            if idx == 3:
                ...
                #feature_enhance = self.fu_conv[idx-1](feature_enhance)     # 完成特征增强后，是否需要Conv，BN，ReLU
                outs_enhance.append(feature_enhance)
                outs.append(resize(input=self.convs_enhance[idx - 1](feature_enhance), size=[64,64], 
                                   mode='bilinear', align_corners=False))
            elif idx == 2:
                feature_enhance = aerfa * edge * feature
                #feature_enhance = self.fu_conv[idx-1](feature_enhance)
                outs_enhance.append(feature_enhance)
