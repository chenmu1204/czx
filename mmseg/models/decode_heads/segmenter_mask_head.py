...
class SegmenterMaskTransformerHead(BaseDecodeHead):
    ...
    def forward(self, inputs):    # inputs(4,384,32,32)
        ...
        patches = F.normalize(patches, dim=2, p=2)            # （4,1024,384）
        cls_seg_feat = F.normalize(cls_seg_feat, dim=2, p=2)  # （4,4,384）

        masks = patches @ cls_seg_feat.transpose(1, 2)        # （4,1024,384）*(4,384,4) → (4,1024,4)
        masks = self.mask_norm(masks)
        masks = masks.permute(0, 2, 1).contiguous().view(b, -1, h, w)

        return masks  # (4,4,32,32) 相对于过了分类器
