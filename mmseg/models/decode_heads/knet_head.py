...
class IterativeDecodeHead(BaseDecodeHead):
    ...
    def __init__(self, num_stages, kernel_generate_head, kernel_update_head,
        ...
        '''思路1：自定义卷积层'''
        # channels1 = [4, 4, 4, 4]  # RSMI数据集
        # channels2 = [4, 4, 4, 4]
        channels1 = [6, 6, 6, 6]  # WHDLD数据集
        channels2 = [6, 6, 6, 6]
    ...
    def forward(self, inputs):
        ...
        outs_enhance.append(stage_segs[3])  # 保留最底层特征
        aerfa = 0.5
        beta = 0.5
        for idx in range(len(stage_segs) - 1):
            idx = 3 - idx        # [3,2,1]
            x1 = stage_segs[idx]     # (64,64,4)
            x2 = stage_segs[idx - 1] # (64,64,4)
            #conv = self.convs_enhance[idx]
            #x1 = conv(x1)  # resize(input=conv(x1), size=x2.shape[2:], mode='bilinear', align_corners=False)  # 先改变通道数，再resize
            ...
            if idx == 3:
                ...
                #feature_enhance = conv(feature_enhance)
