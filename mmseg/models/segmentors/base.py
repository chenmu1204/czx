...
class BaseSegmentor(BaseModel, metaclass=ABCMeta):
    ...
    def postprocess_result(self,
        ...
        # '''添加连通域判断条件'''
        # pre = data_samples[0]._pred_sem_seg.data
        # if (pre==3).sum().item() >= ((pre==1).sum().item() + (pre==2).sum().item()):
        #     data_samples[0]._pred_sem_seg.data = torch.where((pre == 1) | (pre == 2), torch.ones_like(pre)*3, pre)
        # elif (pre==2).sum().item() >= ((pre==1).sum().item() + (pre==3).sum().item()):
        #     data_samples[0]._pred_sem_seg.data = torch.where((pre == 1) | (pre == 3), torch.ones_like(pre)*2, pre)
        # else :
        #     data_samples[0]._pred_sem_seg.data = torch.where((pre == 2) | (pre == 3), torch.ones_like(pre), pre)
