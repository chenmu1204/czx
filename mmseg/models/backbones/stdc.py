...
class STDCContextPathNet(BaseModule):
    ...
    def forward(self, x):
        outs = list(self.backbone(x))                # 从backbone得到分层次的特征：3层（4,256,64,64）（4,512,32,32）（4,1024,16,16）
        ...
        # `outs[0]` is outputted for `STDCHead` auxiliary head.              outs[0] 给 STDCHead的辅助分类器
        # Two feature maps of `arms_out` are outputted for auxiliary head.   arms_out 给 2个FCN的辅助分类器
        # `feat_fuse` is outputted for decoder head.                         feat_fuse 给 解码头
